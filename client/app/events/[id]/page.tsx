...
import { Calendar, MapPin, Users, IndianRupee, Check, ChevronLeft } from "lucide-react";
...
export default function EventDetailsPage() {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      await axios.post(`${BACKEND_URL}/events/${id}/book`, {}, {
        headers: {
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <h1 className="font-display text-5xl text-foreground">{event.name}</h1>
              </div>
              ...
              <div className="space-y-4 pt-4">
                <h3 className="text-xs uppercase tracking-[0.2em] font-bold text-primary">What's Included</h3>
                <div className="grid grid-cols-2 gap-4">
                  {event.amenities.map((a, i) => (
                    <div key={i} className="flex items-center gap-3 text-sm text-foreground bg-card border border-border p-4">
          ...
          <div className="space-y-8">
            <Card className="sticky top-32 border-2 border-primary/10 shadow-xl">
              <CardHeader className="bg-muted/30 pb-8">
                ...
                <CardDescription>Secure your tickets for this event</CardDescription>
              </CardHeader>
              <CardContent className="pt-8 space-y-8">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-foreground font-semibold">{event.venue}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground flex items-center gap-2 uppercase tracking-widest font-medium text-[10px]">
                            <Users size={14} className="text-primary" /> Availability
                        </span>
                        <span className="text-foreground font-semibold">{event.capacity} spots left</span>
                    </div>
                </div>

                <div className="pt-6 border-t border-border flex items-end justify-between">
                    <div className="space-y-1">
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Total Price</span>
                ...
                >
                    {booking ? "Processing..." : event.available ? "Confirm Booking" : "Sold Out"}
                </Button>
